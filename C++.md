...
- 关键字
    ...
    - [static](#static)
    - [volatile](#volatile)
    - [const (定义-指针-类-define-函数-连接-内存-转换)](#const)
    - [new和malloc的区别 (来源-位置-调用-类型安全-分配失败-分配大小-构造函数-重载-扩存-内存泄露)](#new和malloc的区别)
    - [Register](#Register)
    - [inline](#inline)
    - [virtual](#virtual)
    - [C++涉及存储位置的关键字](#涉及存储位置的关键字)
- 基础知识
    - [C++是如何通过虚函数实现多态的](#C++是如何通过虚函数实现多态的) 
    - [为什么析构函数是虚函数](#为什么析构函数是虚函数)
    - [能展开讲讲智能指针吗](#能展开讲讲智能指针吗)
    - [如何判断weak_ptr的对象是否失效](#如何判断weak_ptr的对象是否失效)
    - [什么是段错误](#什么是段错误)
    - [构造、析构函数能抛出异常吗](#构造析构函数能抛出异常吗)
    - [指针的大小](#指针的大小)
    - [堆和栈的区别](#堆和栈的区别)
    - [引用与对象的生命周期](#引用与对象的生命周期)
    - [引用和指针的区别](#引用和指针的区别)
    - [C和C++的强制类型转换](#强制类型转换)
    - [数组与指针的区别](#数组与指针的区别)
- C++内存管理
    - [内存布局](#内存布局)
- 数据结构与算法
    - [vector是怎么实现的](#vector是怎么实现的)
    - [push_back和emplace_back有什么区别](#push_back和emplace_back有什么区别)
    - [sort的内部实现](#STL-sort的内部实现)
- C++11的特性
    - [类型推导](#类型推导)
    - [右值引用](#右值引用)
    - [列表初始化](#列表初始化)
    - [std::function、std::bind、lambda表达式](#function-bind-lambda表达式)
    - [对模板的改进](#对模板的改进)
    - [其它小的变化](#其它小的变化)
    - [final 和 override](#final和override)
- 代码实现
    - [声明一个结构体指针](#声明一个结构体指针)
    - [实现一个二叉树类](#实现一个二叉树类)
    - [好题收藏经典算法](#好题收藏经典算法)
...
##### inline
- inline关键字的使用是为了解决C/C++中频繁调用函数而大量消耗内存空间，也就是栈内存的问题，修饰在函数前表示为内联函数
- 函数内联，就是在每个使用到内联函数的地方都替换展开成函数内的代码语句
- 类中的成员函数和inline：
    - 在类内定义的成员函数默认就是内联的，所以不加inline也跟加了inline的效果一样
    - 在类外定义的成员函数不是内联的，需要在类外定义的时候在函数名前加inline
- inline它不是用于声明的关键字，而是用于实现的关键字，inline必须和函数定义体放在一起才能使函数成为内联，仅将inline放在函数声明前不起任何作用
- 有三种情况是不适合使用inline关键字的：
    - 代码很长。函数内联是以代码膨胀为代价，仅省去了函数调用的开销，而每一处使用到函数内联的地方都要复制代码，所以如果代码很长反而会增大内存的消耗
    - 函数体内包含了复杂的语句比如while、for、switch
    - 函数是递归函数，或者说里面调用了自己本身，比如dfs这种
    - （后面两种情况会导致执行函数内代码的时间远超过函数调用的开销）
- 不过inline的使用对于编译器来说只是一个建议，最后是否真的内联是看编译器的选择，如果编译器觉得不复杂就会进行内联。
- 但是我们在使用的过程中如果分析后发现函数内联不能提高性能，就应该避免使用它

### virtual
- 用于声明虚函数，只要在函数名前加这个关键字，那么这个函数就是个虚函数
- 纯虚函数的定义就是virtual void foo()=0;
    - 纯虚函数的定义就是声明了一个抽象类，所在类必须是一个基类，不能实例化对象，而是用于派生类继承的一个父类
    - 纯虚函数的存在是的派生类必须要重写这个虚函数的方法，这样能保证不同的派生类能有相同的函数，但是又有各自的内部具体实现，达到运行时多态的效果
- 然后对于所有的基类，也就是拿去被继承的类，它的析构函数也必须是虚函数，否则父类的析构函数不会被调用而造成内存泄露
    - 因为当声明了一个基类的指针指向子类的对象的时候，当这个对象的生命周期结束的时候，只会调用基类的析构函数，而如果基类的析构函数不是虚函数的话，就不会调用子类的虚函数，导致内存泄露
    - 所以要在析构函数前加virtual
...
##### C++是如何通过虚函数实现多态的
- 虚函数是C++中用于实现多态的机制，核心就是通过基类访问派生类定义的虚函数达到运行时确认函数的具体调用的效果
- 之所以被称为多态，就是因为运行后程序才知道该调用哪个类的函数
- 虚函数调用的具体细节就是
    - 对于每个继承了虚函数的类或者是内部定义了虚函数的类，都维护了一个虚函数表
    - 更具体点就是，每个虚函数类声明的对象都添加了一个隐藏成员，这个隐藏成员内保存了一个指向虚函数地址数组的指针
    - 这个虚函数地址数组就是一个虚函数表，所以每个虚函数类声明的对象都有一个虚函数指针都指向了相同的虚表
    - 这个虚函数指针叫__vptr，然后再通过虚表调用相应的虚函数
    - 所以从根本上可以说：虚函数是通过指针或者引用来实现多态的
    - 关于类和虚表的关系就是
        - 抽象基类会有一个自己的虚函数表，存储基类的虚函数
        - 如果派生类重写了基类的虚函数，那么基类就会有自己的虚函数表，否则派生类的虚表就是使用的基类的虚表，在这种情况下定义了新的虚函数，函数地址会被添加到基类的虚表中去
- 另外非虚函数的调用是不会经过虚表的

##### 为什么析构函数是虚函数
- 首先是析构函数不一定要是虚函数，因为析构函数声明成虚函数需要额外的虚函数表和虚函数指针，需要占用额外的内存，导致浪费内存
- 然后如果当析构函数所在的类是抽象类或者基类的时候，一定设置成虚函数，因为这样才能在用父类指针指向子类对象的时候，释放这个基类指针也能释放子类的空间，防止内存泄露
...
##### 强制类型转换
- C的强制类型转换就是在对象前加括号，括号里面放要转换到的类型名就行
- C++的强制类型转换有四种
    - const_cast
        - 去掉类型的const或volatile属性
    - static_cast
        - 无条件转换，静态类型转换，类似C风格的强制转换，常用于基础类型比如int、double、float、long long等这些常用类型的相互转换
    - dynamic_cast
        - 有条件转换，动态类型转换，运行时会检查类型安全，转换失败返回NULL
        - 适用于多态类的类型转换，要求基类要有虚函数，即具有多态性
    - reinterpret_cast
        - 适用于不同类型的指针之间的强制转换
        - 仅重新解释类型，但没有进行二进制的转换

##### 数组与指针的区别
- 不加const关键字的情况下，指针是变量，存储的是对象的地址，而数组是常量，实际上相当于一个指针常量
- 存储空间不同
    - 数组存储在静态存储区和栈上
    - 指针不仅仅是静态存储区和栈上，根据指针的类型不同而不同
- 回收操作不同
    - 因为存储空间的不同，会导致一个回收的操作不同
    - 对于数组而言，如果是存储在静态存储去也就是全局数组的话，数组的生命周期和程序一样，当程序结束的时候数组就会回收；如果是在栈上的数组，也就是函数内声明的局部数组，那么在函数退出时就会回收栈内存
    - 而指针如果是通过malloc声明的，则需要通过free来释放内存，如果是通过new声明的，则需要通过delete来回收，否则会发生内存泄露
- 所占空间内存不同
    - 数组在内存中是一端连续的内存空间，比如你声明了a[100];那么你输出sizeof(a)/sizeof(int)会输出100
    - 而指针的大小是根据计算机或者说编译器的平台的位数确定的，32位计算机，指针的就是4,64位就是8.所以你输出sizeof(b)/sizeof(int)会输出4或者8
- 使用不同
    - 数组的使用可以使用索引很方便地去修改,是直接访问
    - 而指针需要改变指针指向的对象的值来改变访问指针时获得的值,是间接访问
- 类型不同
    - 数组是静态的，自身就是数组名
    - 而指针是动态的，可以动态地改变指向的对象，也能通过改变指针指向的对象的值来改变访问指针时获得的值。所以指针可以说是匿名的
- 初始化不同
    - 数组在初始化后，可以修改
    - 而如果指针进行了初始化，比如char * a = "asdf";那么这个指针会被定义为只读，如果试图通过指针去修改这个字符串的值，程序就会报错
...
##### 声明一个结构体指针
'''C++
node *Begin=(node *)malloc(sizeof(node)),*another=(node *)malloc(sizeof(node));//第1种
node *Begin=new node(3),*another=new node(5);//第2种
'''
