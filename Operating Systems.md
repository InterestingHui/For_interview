...
- 管道分为匿名管道PIPE和命名管道FIFO
    - 匿名管道(Pipe)
    	- 管道是**半双工**的，数据是**单向流动**的
    	- 比如说Linux中的'|'就是匿名管道，'|'的功能就是左边文件的输入是右边文件的输入。所以匿名管道并没有用什么东西保存，也就是说匿名管道**使用完就会销毁**。
...
4. 共享内存
    ...
    - 共享内存是**最快**的通信方式，因为它不需要从用户态到内核态的切换，所以通信的时候只需要两次拷贝：一次是A进程给内存中写数据，另一次是B进程从内存中读取数据。
    ...
    - 共享内存通常和信号量一起使用，实现进程间通信和同步。
5. 信号量(Semaphore)：
    - 信号量是一个**计数器**不同于其它通信方式，它不存储通信数据，而是进程间同步的方式，用来控制多个进程对共享资源的访问，如果真的传递数据需要结合共享内存的方式。
    - 信号量是一种**锁机制**，用来防止某个进程访问共享资源的时候其它进程也访问这个资源，实现进程间同步
    - 程序对信号量的操作都是**原子**操作，信号量是基于操作系统的PV操作，P操作使得信号量-1然后再判断信号量是否小于零，如果小于0就先阻塞进程，再让进程进入到信号量的等待队列中，再进行进程调度；否则就让进程继续运行。V操作使信号量+1，然后判断信号量是否小于等于0，如果小于等于0，就从信号的等待队列释放一个进程，然后再返回原进程或者进行进程调度。
    - 最简单的信号量就是只能取0和1的变量，叫二值信号量；如果能取多个整数的信号量被称为通用信号量，Linux下的信号量函数都是在通用的信号量数组上操作的。
6. 套接字(Socket)
