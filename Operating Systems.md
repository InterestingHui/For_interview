...
<div>
	<ul>
	 <li>详细讲讲共享内存
		 共享内存是进程间通信最快的一种方式。</n>
	 （i）从宏观上它指的是两个进程通过直接访问同一个逻辑地址而实现进程间通信的方式。</n>
	 （ii）微观上，每个进程都有各自的PCB（进程控制块）、地址空间、页表，页表通过MMU（内存管理单元）管理，页表上存储虚拟地址到物理地址的映射，而共享内存就是两个进程的各自页表上的一个虚拟地址指向了同一个物理地址，这样这个物理地址中就有一块共同的内存区，这就是共享内存。</n>
	 （iii）操作系统对共享内存没有添加自动同步机制，所以需要自己设置同步机制，例如采用信号量。</n>
	 （iv）共享内存最快的原因是因为共享内存只需要两次拷贝：A 进程给内存中写数据，B 进程从内存中读取数据。</n>
