...
<details><summary>详细讲讲<b>共享内存</b></summary>
	
- 共享内存是进程间通信最快的一种方式。
- 从宏观上它指的是两个进程通过直接访问同一个逻辑地址而实现进程间通信的方式。
- 微观上，每个进程都有各自的PCB（进程控制块）、地址空间、页表，页表通过MMU（内存管理单元）管理，页表上存储虚拟地址到物理地址的映射，而共享内存就是两个进程的各自页表上的一个虚拟地址指向了同一个物理地址，这样这个物理地址中就有一块共同的内存区，这就是共享内存。
- 操作系统对共享内存没有添加自动同步机制，所以需要自己设置同步机制，例如采用信号量。
- 共享内存最快的原因是因为共享内存只需要两次拷贝：一次是A 进程给内存中写数据，另一次是B 进程从内存中读取数据。
</details>

<details><summary>什么是<b>虚拟内存</b></summary>
	
- 虚拟内存就是对于程序来说，逻辑上有很大内存，但实际有一部分是存储在磁盘上的内存。虚拟内存一般是需要用到磁盘这部分内存的时候，再引出缺页中断，然后把磁盘上的内存换上物理内存，从而使得逻辑上能使用更大的内存。
</details>

<details><summary>讲讲<b>继承</b>和<b>重载</b></summary>

- 继承是指用一个已有的类去定义新的类，被继承的叫父类或者说基类，继承的叫子类或者说派生类。
- 一个子类可以继承多个父类，然后子类能继承父类中非private声明下的属性
- 重载的话，重载有两种：一种是函数重载，另一种是运算符重载。函数重载就是，一个类下，同样名称的函数对于不同的参数列表有不同的反馈；而运算符重载其实是特殊的函数重载，可以视为函数名为operator+运算符的函数重载，在自定义排序规则或者是为了简化代码的时候使用。
- 重载是多态的一种
</details>
...
- 管道分为匿名管道PIPE和命名管道FIFO
    - 匿名管道(Pipe)
    	- 管道是**半双工**的，数据是**单向流动**的；所以如果双方都要进行读写通信的话，需要建立起**两个**管道。
    	- 例如Linux中的**'|'**就是匿名管道，因为它并没有用什么东西保存，所以匿名管道使用完就会**销毁**。
    	- 需要注意的是匿名管道只能用于具有**亲缘关系**的进程
    	- 再说下匿名管道通信的**过程**：
    		1. 首先匿名管道的创建相当于定义一个pipe函数，传入参数是一个fd数组，这个数组只有两个元素，fd[0]和fd[1]，一个是管道写端，另一个是管道读端。让父进程的两个文件描述符指向这管道的两端。
    		2. 然后父进程fork出子进程，子进程也有两个文件描述符指向同一个管道
    		3. 父进程关闭fd[1],子进程关闭fd[0],即父进程关闭管道读端，子进程关闭管道写端，这是因为管道是单向的，这样使得数据从写端流入，读端流出，完成进程间通信单向通信
    - 命名管道(FIFO)
	- 命名管道通过mkfifo命令创建，用一个自己命名的变量或者说文件存储，因为Linux下都视为文件，这个文件就是命名管道。
  	- 命名管道的通信没有亲缘关系的限制，只要能访问mkfifo创建的文件的进程就能跟本进程通信，所以命名管道没有亲缘限制
        - 另外因为它的读写数据的方式是先进先出FIFO，所以又把这种管道通信称为FIFO管道通信
2. 消息队列
    - 消息队列是消息的**链表**，放在内核中并且有标识符标识
    - 消息队列和命名管道有点像,都可以支持两个不相关的进程通信,但是消息队列**能传递的信息更多**
3. 信号(Signal)
    - 信号传递的消息特别少，所以用信号进行通信仅用于通知接收进程某个事件已经发生
4. 共享内存
    - 共享内存就是**映射一段内存**，由一个进程创建，然后多个进程都能访问。
    - 共享内存是**最快**的通信方式，因为它不需要从用户态到内核态的切换，所以通信的时候只需要两次拷贝。
5. 信号量(Semaphore)：
    - 信号量是一个**计数器**，也是进程间同步的方式，用来控制多个进程对共享资源的访问
    - 信号量是一种**锁机制**，用来防止某个进程访问共享资源的时候其它进程也访问这个资源
6. 套接字(Socket)
    - 这里的套接字通常不是指网络中的套接字，而是指本地的套接字，其它和网络套接字相同
</details>
