...
- 匿名管道(Pipe)
    - 管道是半双工的，数据是单向流动；需要双方通信时，需要建立起两个管道。
    - 例如Linux中的'|'就是匿名管道，因为它并没有用什么东西保存，所以匿名管道使用完就会销毁。
    - 需要注意的是匿名管道只能用于具有亲缘关系的进程
    - 再说下匿名管道通信的过程：
    	- 首先匿名管道的创建相当于一个pipe函数，传入参数是一个fd数组，这个数组只有两个元素，fd[0]和fd[1]
    	- 父进程fork出子进程，子进程也有两个文件描述符指向同一个管道
    	- 父进程关闭fd[0],子进程关闭fd[1],即父进程关闭管道写端，子进程关闭管道读端， 这是因为管道是单向的，这样使得数据从写端流入，读端流出，完成进程间通信

- 命名管道(FIFO)
- 命名管道的通信没有亲缘关系的限制
  通过mkfifo命令创建，用一个变量或者说文件（因为Linux下都视为文件）存储，这个文件就是命名管道。
  所以只要能这个文件的进程就能跟本进程通信，所以命名管道没有亲缘关系的限制
  它的读写数据的方式是先进先出FIFO，所以又把这种管道成为FIFO管道通信
2. 消息队列
3. 信号(Signal)
4. 共享内存
