<h3>MySQL</h3>
<detail><summary>1.MySQL 一条SQL语句的长度大小限制</summry><li>sql语句的长度大小默认限制是1MB，但是可以通过设置来达到最大1GB的长度大小限制</details>
<detail><summary>2.MySQL 行长度限制</summary><li>MySQL中一行数据的字段总长最长不能超过65535字节，所以每个字段的允许长度先是满足该类型规定的最大长度在要满足行数据的最长长度限制。</details>
# 数据库
