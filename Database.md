...
- SQL**更新语句**的执行过程，可以说是7步:
    - 这里主要是说一下采用InnoDB引擎的SQL更新语句的执行过程，更新语句需要用到两个日志：
        - 一个是binlog归档日志是MySQL server层的日志，是逻辑日志，记录的是SQL语句的原始逻辑，是追加写入的，不会覆盖之前的日志
        - 另一是relog重做日志，是InnoDB特有的日志，具有固定大小，可以进行配置，比如配置为一组4个文件，每个文件1GB,relog是循环写入的，写到末尾就会从头开始覆盖
